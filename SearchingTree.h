...
namespace my_bst {
    ...
    public:
        ...
        explicit TreeIter(PtrType *root) {
            this->fill(root);
            this->next();
        }

        TreeIter(PtrType *ptr, std::stack<PtrType *> stack) {
            m_ptr = ptr;
            m_node_stack = stack;
            fill(ptr->right);
        }

        static TreeIter begin_iter(PtrType *root) {
            auto iter = TreeIter(root);
            return iter;
        }

        static TreeIter end_iter() {
            auto iter = TreeIter();
            iter.m_is_end = true;
            return iter;
        }
        ...
        friend bool operator==(const TreeIter &a, const TreeIter &b) {
            return a.m_ptr == b.m_ptr || (a.m_is_end && b.m_is_end);
        };
